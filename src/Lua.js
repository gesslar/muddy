...
export default class Lua {
  ...
  static fileName(name) {
    const collapsed = name.replaceAll(/\s/g, "_")

    // sanitize() rejects an empty string; a name that is empty or all-whitespace
    // has no usable filename, so short-circuit to "" rather than throw.
    return collapsed ? FileSystem.sanitize(collapsed) : ""
  }
   ...
   */
  static longString(value) {
    let eq = ""

    while(value.includes(`]${eq}]`))
      eq += "="
